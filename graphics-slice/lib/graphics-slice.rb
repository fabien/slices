  ...
  module GraphicsSlice
    ...
    def self.setup_router(scope)
      scope.to(:controller => 'textim') do |textim|
        ...
        textim.match("/textim/:preset", :method => :delete).to(:action => 'delete_preset_items').name(:textim_delete_preset)
        # remove all cached textim images
        textim.match("/textim", :method => :delete).to(:action => 'delete_all_items').name(:textim_delete_all)
      end
      scope.to(:controller => 'images') do |images|
        ...
        # get info for the specified image
        images.match("/images-info/:preset/:checksum/:storage/:original_ext/:filename.:format", :method => :get, 
          :checksum => /[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}/, :storage => /[a-f0-9]+/, :original_ext => /[a-f0-9]+/, :filename => /[^\.]+/).to(:action => 'info')
        images.match("/images/index.html").to(:action => 'index')   
        # get an image url back for the given source image and preset
        # curl -X POST -d 'merb.jpg' http://localhost:4000/media/images/default
        # RestClient.post('http://admin:secret@localhost:4000/media/images/default?format=gif', 'merb.jpg')
        images.match("/images/:preset", :method => :post).to(:action => 'image_location').name(:image_location)  
        # remove all textim images generated by a certain preset
        images.match("/images/:preset", :method => :delete).to(:action => 'delete_preset_items').name(:images_delete_preset)
        # remove all images generated by a certain preset - using asset references in the preset context
        images.match("/images", :method => :delete).to(:action => 'delete_all_items').name(:images_delete_all)
      end
      scope.to(:controller => 'external') do |images|
        ...
        images.match("/external/:preset/:checksum/:storage/:original_ext/:filename.:format", :method => :delete, 
          :checksum => /[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}/, :storage => /[a-f0-9]+/, :original_ext => /[a-f0-9]+/, :filename => /[^\.]+/).to(:action => 'delete')
        # get info for the specified image
        images.match("/external-info/:preset/:checksum/:storage/:original_ext/:filename.:format", :method => :get, 
          :checksum => /[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}\/[a-f0-9]{8}/, :storage => /[a-f0-9]+/, :original_ext => /[a-f0-9]+/, :filename => /[^\.]+/).to(:action => 'info')
        images.match("/external/index.html").to(:action => 'index')
        # get an image url back for the given source image and preset
        # curl -X POST -d '/3112/3195238780_fd89b49831.jpg' http://localhost:4000/media/external/default?storage=flickr
        images.match("/external/:preset", :method => :post).to(:action => 'image_location').name(:external_image_location)
        # remove all textim images generated by a certain preset
        images.match("/external/:preset", :method => :delete).to(:action => 'delete_preset_items').name(:external_delete_preset)
        # remove all images generated by a certain preset - using asset references in the preset context
        images.match("/external", :method => :delete).to(:action => 'delete_all_items').name(:external_delete_all)
      end
    end
